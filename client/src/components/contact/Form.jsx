...
const [b0Spm, setB0Spm] = useState('');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
      ...
      const emptyFields = requiredFields.filter(item => (!item.value && item.required));
    ...
    <main>
      <div id='contact' className='w-full flex flex-wrap-reverse items-center justify-evenly'>
        <form className='w-[450px] flex flex-col items-center m-0'>
          ...
          <input className='w-1/4 border-2 border-blue rounded-lg mb-3' type='text' placeholder='Name' value={name} onChange={ (e) => setName(e.target.value) } required />
          <input className='w-2/5 border-2 border-blue rounded-lg mb-3' type='email' placeholder='Email' value={email} onChange={ (e) => setEmail(e.target.value) } required />
          <textarea className='w-3/4 border-2 border-blue rounded-lg mb-3' placeholder='Message' value={message} rows="8" onChange={ (e) => setMessage(e.target.value) } required/>
